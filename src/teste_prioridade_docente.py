...
from estrategia_reserva import PoliticaPrioridadeDocente
from gerenciador_reservas import GerenciadorDeReservas

def main():
    print("=" * 60)
    print("TESTE DO SISTEMA DE RESERVAS - POLÍTICA: PRIORIDADE DO DOCENTE")
    print("=" * 60)
    ...
    # 3. Configurar política Prioridade do Docente
    ReservaFactory.definir_politica(PoliticaPrioridadeDocente())
    print("\nPolítica de reserva configurada: PRIORIDADE DO DOCENTE")
    ...
    # Reserva 3: Professor na sala I-1 das 10:00 às 12:00 (sem conflito)
    inicio3 = datetime(2026, 5, 25, 10, 0)
    fim3 = datetime(2026, 5, 25, 12, 0)
    print(f"{professor.nome} → Sala {indiv1.numero_sala} | {inicio3.strftime('%H:%M')} - {fim3.strftime('%H:%M')}")
    r3 = ReservaFactory.criar_reserva(professor, indiv1, inicio3, fim3)
